package io.academia.payment.controller;

import io.academia.payment.config.ConfigPayment;
import io.academia.payment.dto.PaymentRequest;
import io.academia.payment.dto.PaymentRes;
...
public class PaymentController {
    ...
    public ResponseEntity<?> createPayment(HttpServletRequest request, @RequestBody PaymentRequest paymentRequestDto) throws UnsupportedEncodingException {
        String studentIdString = paymentRequestDto.getStudentId().toString();
        Double doubleCart = webBuilder.build()
                .get()
                .uri("http://localhost:8900/api/cart/" + studentIdString + "/total")
                .retrieve()
    ...
    public ResponseEntity<PaymentResponse> invoice(
        ...
        PaymentResponse paymentResponse = service.createPayment(paymentRequest, bankCode, cardType, orderInfo, transactionStatus);
